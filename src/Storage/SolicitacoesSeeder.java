...
public class SolicitacoesSeeder {
    private final SolicitacaoService solicitacaoService;

    public SolicitacoesSeeder(SolicitacaoService solicitacaoService) {
        this.solicitacaoService = solicitacaoService;
    }

    public ResultadoSeeder inicializar(String arquivoDados) {
        int carregadas = solicitacaoService.carregarSolicitacoes(arquivoDados);
        if (carregadas > 0) {
            return new ResultadoSeeder(false, carregadas);
        }

        popularDadosIniciais();
        solicitacaoService.salvarSolicitacoes(arquivoDados);
        return new ResultadoSeeder(true, 10);
    }
    ...
    public static class ResultadoSeeder {
        private final boolean seedCriada;
        private final int quantidade;

        public ResultadoSeeder(boolean seedCriada, int quantidade) {
            this.seedCriada = seedCriada;
            this.quantidade = quantidade;
        }

        public boolean isSeedCriada() {
            return seedCriada;
        }
